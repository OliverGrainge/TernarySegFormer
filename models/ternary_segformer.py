...
def ternify_model(model): 
    for name, layer in model.named_modules(): 
        if isinstance(layer, torch.nn.Linear): 
            has_bias = layer.bias is not None
            qlinear = BitLinear(in_features=layer.in_features, out_features=layer.out_features, bias=has_bias)
            qlinear.weight.data = layer.weight.data 
            if has_bias: 
                qlinear.bias.data = layer.bias.data
            ...
            setattr(parent_module, name_parts[-1], qlinear)

def get_ternary_segformer_model_and_processor(pretrained=False):
    if pretrained: 
        raise Exception("Pretrained model not supported")
    else: 
